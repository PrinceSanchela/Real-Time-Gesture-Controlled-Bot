...
# DEVICE SETUP (FORCED CPU)
DEVICE = "cpu"

print("================================")
print(f"RUNNING ON DEVICE: {DEVICE.upper()}")
print(f"CPU Threads Allocated: {torch.get_num_threads()}")
print("================================")

# Kept at 320 for low latency on CPU
IMG_SIZE = 320
...
# Command heartbeat interval (seconds) to prevent overloading the network
...
try:
    while True:
        ...
        # YOLO Inference forced on CPU
        results = model(
            frame, conf=0.5, verbose=False, device=DEVICE, imgsz=IMG_SIZE
        )
        ...
        if boxes is not None and len(boxes) > 0:
            ...
            for box in boxes:
                b_cls = int(
                    box.cls.item() if hasattr(box.cls, "item") else box.cls[0]
                )
                ...
                annotator.box_label(
                    box.xyxy[0], label_text, color=colors(b_cls, True)
                )
        ...
        current_time = time.time()
        instant_fps = (
            1 / (current_time - prev_time) if current_time != prev_time else 0
        )
        ...
        if (current_command != last_gesture) or (
            current_time - last_cmd_send_time > HEARTBEAT_INTERVAL
        ):
